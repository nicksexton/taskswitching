...
#include "3task_model_koch.h" // model
...
int main (int argc, char *argv[]) {
  ...
  simulation->model = pdp_model_create (0, "3task_koch_1"); 
  three_task_koch_conflict_parameters_htable_set_default (simulation->model_params_htable);


  // define function pointer to model_run function 
  int (*model_run)(pdp_model*, ThreeTaskSimulation*);
  model_run = &three_task_model_koch_conflict_run; // (model-specific, def in 3task_model_gs.c)

  int (*model_reinit)(pdp_model*, init_type, ThreeTaskSimulation*);
  model_reinit = &three_task_model_koch_conflict_reinit; // (model-specific, def in 3task_model_gs.c)
  ...
  three_task_koch_conflict_parameters_import_commit (param_config_file, simulation->model_params_htable);
  ...
  init_model_koch_conflict_simulation (simulation->model, simulation->model_params_htable); 
