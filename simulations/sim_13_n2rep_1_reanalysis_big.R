...
# block size 200 
 ...
 data.1 = read.delim("sim_13_modelv1_gridsearch_8x8pt1.txt", sep=c("\t"), strip.white=TRUE, header=TRUE, stringsAsFactors=FALSE)

 data.2 = read.delim("sim_13_modelv1_gridsearch_8x8pt2.txt", sep=c("\t"), strip.white=TRUE, header=TRUE, stringsAsFactors=FALSE)

data.raw <- rbind (data.1, data.2)
...
data.sim13b.comparerepetitions.gain70 <- subset (data.sim13b.comparerepetitions, data.raw$conflict.bias== -6 & data.raw$conflict.gain== 70)

############### Switch costs vs. n2rep costs ############

data.sim13b.gain70.bias6 <- subset (data.raw, data.raw$conflict.gain == 70 & data.raw$conflict.bias == -6)

# Data set for looking at sc and n2rc varying with confl-td weight
data.sim13b.sc <- subset (data.sim13b.gain70.bias6, select=c("conflict.tdwt", "conflict.gain", "conflict.bias", "sc"))
names (data.sim13b.sc) <- c("conflict.tdwt", "conflict.gain", "conflict.bias", "cost")
data.sim13b.sc$dv <- rep("sc", nrow(data.sim13b.sc))
data.sim13b.n2rc <- subset (data.sim13b.gain70.bias6, select=c("conflict.tdwt", "conflict.gain", "conflict.bias", "n2rc"))
...
#  stat_summary(fun.data = mean_cl_boot, geom = "errorbar", position = position_dodge(width = 0.90), width = 0.2) + 
  ...
  ggtitle("Sim13b, bias -6 & gain 70, effect of TDWT on switch & n2rep costs")

imageFile <- file.path("~/Dropbox/PhD/Thesis/simulation_results/simulation_13", "sim_13b_defaults_varyweight_costs_x2.png")
...
#  geom_errorbar (aes(ymin = mean - 
  ...
  ggtitle("RT for repetitions (1SW, 0SW) vs. nonrepetitions (2SW, ALT), gain 70 bias 6")


imageFile <- file.path("~/Dropbox/PhD/Thesis/simulation_results/simulation_13", "sim_13b_defaults_rts_aggregate_x2.png") 
ggsave(filename=imageFile, width = 300, height = 300, units = "mm")

############## RT BY CONDITION (single gain value) #########
linegraph <- ggplot (data.sim13b.comparerepetitions.gain70, aes(x=conflict.tdwt, y=RT, group=dv, colour=dv))
linegraph +
  stat_summary(fun.y = mean, geom = "line", position = "dodge") +
  labs (x = "Inhibitory Weight", y = "RT", group = "DV") +
  ggtitle("RTs individually by condition, gain 70 bias 6")

imageFile <- file.path("~/Dropbox/PhD/Thesis/simulation_results/simulation_13", "sim_13b_defaults_rts_all_x2.png")
ggsave(filename=imageFile, width = 300, height = 300, units = "mm")
