...
data.raw <- read.delim("sim_17_data.txt", header=FALSE, sep=c("", ":"), col.names=labels.data)
...
labels.lookup = c("trialid", "sequence", "cue_sequence", "trial.pos", "reps.allowed", "congruency_trial")
 data.lookuptable = read.delim("sim_17_lookup.txt", header = FALSE, col.names=labels.lookup)

data <- merge(data.lookuptable, data, by="trialid", sort=FALSE)


# proportion of each trial type:
with (data.lookuptable, table(sequence, reps.allowed))

# make sure blocks are not duplicates???
labels.trials.conf = c("blockid", "trialid", "cue", "stim.A", "stim.B", "stim.C", "param1", "param2", "blah")
 data.trials.conf = read.delim("sim_17_trials.conf", header = FALSE, col.names=labels.trials.conf)

data.tasks <- merge (data.lookuptable, data.trials.conf, by="trialid", sort=FALSE)

with (data.tasks, table(blockid, cue))

                                        # are blocks all identical??!!
head (subset (data.tasks, data.tasks$blockid == "1_NRP"))
head (subset (data.tasks, data.tasks$blockid == "0_NRP"))
head (subset (data.tasks, data.tasks$blockid == "10_NRP"))
