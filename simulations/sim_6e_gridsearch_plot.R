...
rt.colour.scale <- c("dodgerblue2", "magenta", "red", "darkred")
...
plot.heatmap.rt.0SW <- function (data, condition.title) {
  ...
  rt.0SW + geom_raster() +
    facet_wrap( ~ conflict.tdwt) +                                        
    scale_fill_gradientn(colours=rt.colour.scale, na.value="black") +
    ggtitle(paste (condition.title,
                   "\nParameter space for conflict parameters\n RT (0SW condition)") ) +
  labs(fill="log RT") +
  theme (legend.position=c(0.87,0.1))
...
plot.heatmap.errors.0SW <- function (data, condition.title) {
#Errors(1,2) in 0SW condition
  ...
  error.rates + geom_raster() +
    facet_wrap( ~ conflict.tdwt) +                                        
    scale_fill_gradientn(colours=c("black", "darkred", "red", "orange", "yellow"), na.value="white", limits=c(0,0.6)) +
    ggtitle(paste (condition.title,
...
plot.heatmap.errors.3.ALT <- function (data, condition.title) {
#Errors(3) in ALT condition
  
  error.rates <- ggplot(data, aes(x=conflict.gain, y=conflict.bias, fill=err.3.ALT))
  error.rates + geom_raster() +
    facet_wrap( ~ conflict.tdwt) +                                        
