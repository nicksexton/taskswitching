...
#
data.sim13b.rt <- rbind (data.sim13b.0SW, data.sim13b.1SW, data.sim13b.2SW, data.sim13b.ALT)


# Now plot RTs per sequence
linegraph <- ggplot (data.sim13b.rt, aes(x=conflict.tdwt, y=RT, group=dv, colour=dv))
linegraph +
...
imageFile <- file.path("~/Dropbox/PhD/Thesis/simulation_results/simulation_13", "sim_13b_rts_all.png")
ggsave(filename=imageFile, width = 400, height = 100, units = "mm")


############### OVERALL RTS ##############################
# RTs overall (all 4 conditions)
linegraph <- ggplot (data.sim13b.rt, aes(x=conflict.tdwt, y=RT))
linegraph +
  stat_summary(fun.y = mean, geom = "line", position = "dodge") +
